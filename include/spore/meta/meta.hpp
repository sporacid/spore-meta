...
namespace spore
{
    ...
    namespace meta
    {
        ...
        constexpr void for_each_base(func_t&& func)
        {
            constexpr meta_type type = get_type<value_t>();
            tuples::for_each<type.bases>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_field(func_t&& func)
        {
            constexpr meta_type type = get_type<value_t>();
            tuples::for_each<type.fields>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_function(func_t&& func)
        {
            constexpr meta_type type = get_type<value_t>();
            tuples::for_each<type.functions>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_constructor(func_t&& func)
        {
            constexpr meta_type type = get_type<value_t>();
            tuples::for_each<type.constructors>(std::forward<func_t>(func));
        }

        template <meta_enum_enabled value_t, typename func_t>
        constexpr void for_each_value(func_t&& func)
        {
            constexpr meta_enum enum_ = get_enum<value_t>();
            tuples::for_each<enum_.values>(std::forward<func_t>(func));
        }
        ...
        {
            constexpr meta_type type = get_type<value_t>();
            tuples::for_each<type.attributes>(std::forward<func_t>(func));
        }
        ...
        {
            constexpr meta_enum enum_ = get_enum<value_t>();
            tuples::for_each<enum_.attributes>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_extension(func_t&& func)
        {
            constexpr meta_type type = get_type<value_t>();
            tuples::for_each<type.extensions>(std::forward<func_t>(func));
        }
        ...
        {
            constexpr meta_enum enum_ = get_enum<value_t>();
            tuples::for_each<enum_.extensions>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_base(const value_t&, func_t&& func)
        {
            for_each_base<value_t>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_field(const value_t&, func_t&& func)
        {
            for_each_field<value_t>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_function(const value_t&, func_t&& func)
        {
            for_each_function<value_t>(std::forward<func_t>(func));
        }

        template <meta_type_enabled value_t, typename func_t>
        constexpr void for_each_constructor(const value_t&, func_t&& func)
        {
            for_each_constructor<value_t>(std::forward<func_t>(func));
        }

        template <meta_enum_enabled value_t, typename func_t>
        constexpr void for_each_value(const value_t&, func_t&& func)
        {
            for_each_value<value_t>(std::forward<func_t>(func));
        }

        template <meta_enabled value_t, typename func_t>
        constexpr void for_each_attribute(const value_t&, func_t&& func)
        {
            for_each_attribute<value_t>(std::forward<func_t>(func));
        }

        template <meta_enabled value_t, typename func_t>
        constexpr void for_each_extension(const value_t&, func_t&& func)
        {
            for_each_extension<value_t>(std::forward<func_t>(func));
        }
