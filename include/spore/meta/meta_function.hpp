...
namespace spore
{
        ...
        meta_tuple<attributes_t...>>
    {
        ...
        constexpr bool invokable_with() const
        {
            // clang-format off
            return requires
            {
                { std::declval<function_t>().operator()(std::declval<args_t>()...) } -> std::same_as<return_t>;
            };
            // clang-format on
        }
        ...
        constexpr bool invokable_with() const
        {
            ...
            return requires
            {
                { std::declval<function_t>().template operator()<param_t, params_t...>(std::declval<args_t>()...) } -> std::same_as<return_t>;
            };
            // clang-format on
        }
        ...
        constexpr bool invokable_with() const
        {
            ...
            return requires
            {
                { std::declval<function_t>().template operator()<param_v, params_v...>(std::declval<args_t>()...) } -> std::same_as<return_t>;
            };
            // clang-format on
        }
